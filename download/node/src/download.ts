...
cmd.option(
    ...
    .option(
        '--source-method <s>',
        'SOURCE_METHOD. If "url" then only the URL is used. If "round-robin" then URL plus any URL_# environment variables are used. If "assemble" then URL plus .# per concurrency are assembled. If "native" then only the URL is used and no concurrency is available. Defaults to "url".'
    )
    .parse(process.argv);
...
async function readBlobNative() {
    return new Promise((resolve, reject) => {
        const file = fs.createWriteStream('output.file');
        logger.info(`getting from ${URL} @ ${performance.now()}...`);
        https
            .request(
                `${URL}${STORAGE_SAS}`,
                {
                ...
                res => {
                    logger.verbose(
                        `first byte received @ ${performance.now()}...`
                    );
                    if (
                        res.statusCode &&
                        res.statusCode >= 200 &&
                        ...
                        file.on('close', () => {
                            logger.verbose(
                                `download completed @ ${performance.now()}...`
                            );
                            resolve();
                        });
                    } else {
