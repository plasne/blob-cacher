...
cmd.option(
    ...
    .option(
        '-f, --file <s>',
        '[REQUIRED] FILE. The path to the file to be uploaded.'
    )
    .option(
        '-a, --account <s>',
    ...
    .option(
        '-c, --container <s>',
        '[REQUIRED] CONTAINER. The name of the storage account container.'
    )
    .option(
        '-s, --sas <s>',
    ...
    .option(
        '-p, --chunks <i>',
        'CHUNKS. Upload the blob in segments so it can potentially be downloaded faster. Defaults to "1".',
        parseInt
    )
    .parse(process.argv);
...
async function writeChunk(url: string, buffer: Buffer) {
    return axios({
        method: 'put',
        url: `${url}${STORAGE_SAS}`,
        headers: {
...
async function startup() {
    try {
        ...
        if (FILE && STORAGE_ACCOUNT && CONTAINER && STORAGE_SAS) {
            ...
            logger.error(
                'You must specify FILE, STORAGE_ACCOUNT, CONTAINER and STORAGE_SAS.'
            );
            process.exit(1);
        }
        ...
        for (let i = 0; i < CHUNKS; i++) {
            ...
            await writeChunk(
                `https://${STORAGE_ACCOUNT}.blob.core.windows.net/sized/part.${i}`,
                buffer
            );
        }
    } catch (error) {
